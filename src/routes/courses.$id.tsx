...
import { Clock, Users, Star, Play, CheckCircle2, Circle, FileText, Code2, HelpCircle, Video } from "lucide-react";
    ...
    >
      <div className="p-8 max-w-5xl">
        <div className="flex items-start gap-6">
          <div className="size-20 rounded-md flex items-center justify-center text-4xl flex-shrink-0" style={{ background: `${course.color}25`, color: course.color }}>{course.icon}</div>
          <div className="flex-1">
            <div className="text-xs font-mono text-muted-foreground">{course.category} · {course.level}</div>
            <h1 className="text-3xl font-bold mt-1">{course.title}</h1>
            <p className="text-muted-foreground mt-2">{course.description}</p>
            <div className="mt-4 flex flex-wrap gap-4 text-sm text-muted-foreground">
              <span className="flex items-center gap-1"><Clock className="size-4" /> {course.hours} hours</span>
              <span className="flex items-center gap-1"><Users className="size-4" /> {course.students.toLocaleString()} students</span>
              <span className="flex items-center gap-1"><Star className="size-4 fill-yellow-400 text-yellow-400" /> {course.rating}</span>
              <span>{totalLessons} lessons</span>
            </div>
            <div className="mt-6 flex gap-3">
              <Link
                to="/learn/$courseId/$lessonId"
                params={{ courseId: course.id, lessonId: course.modules[0].lessons[0].id }}
                className="inline-flex items-center gap-2 bg-primary text-primary-foreground px-5 py-2.5 rounded-md hover:opacity-90"
              >
                <Play className="size-4 fill-current" /> {progress > 0 ? "Continue" : "Start course"}
              </Link>
              <button className="border border-border bg-secondary px-5 py-2.5 rounded-md hover:bg-accent text-sm">Bookmark</button>
            </div>
            {progress > 0 && (
              <div className="mt-6">
            ...
            {course.modules.map((m, idx) => (
              <div key={m.id} className="border border-border bg-card rounded-md overflow-hidden">
                <div className="px-5 py-3 bg-secondary border-b border-border flex items-center justify-between">
                  <div>
                    <div className="text-xs font-mono text-muted-foreground">module {String(idx + 1).padStart(2, "0")}</div>
                  ...
                  {m.lessons.map((l) => (
                    ...
                    >
                      ...
                      <div className="flex-1">
                        ...
                        <div className="text-xs text-muted-foreground capitalize">{l.type} · {l.duration}</div>
                      </div>
...
function LessonIcon({ type }: { type: string }) {
  const Icon = type === "video" ? Video : type === "quiz" ? HelpCircle : type === "coding" ? Code2 : FileText;
